...
class ProbEstimator:
    def __init__(self, args):
        self.real_crits = []
        self.fake_crits = []
        # json_files = glob.glob(os.path.join(args.ref_path, '*.json'))
        # print(f'ProbEstimator: 找到 {len(json_files)} 个 JSON 文件。')
        # for result_file in json_files:
        #     try:
        #         with open(result_file, 'r') as fin:
        #             res = json.load(fin)
        #             if 'predictions' in res:
        #                 self.real_crits.extend(res['predictions'].get('real', []))
        #                 self.fake_crits.extend(res['predictions'].get('samples', []))
        #             else:
        #                 print(f"警告：在 {result_file} 中未找到 'predictions' 键。")
        #     except Exception as e:
        #         print(f"读取 {result_file} 时出错: {e}")
        # print(f'ProbEstimator: 总共 {len(self.real_crits) + len(self.fake_crits)} 个样本。')
        for result_file in glob.glob(os.path.join(args.ref_path, '*.json')):
            with open(result_file, 'r') as fin:
                res = json.load(fin)
                self.real_crits.extend(res['predictions']['real'])
    ...
    def crit_to_prob(self, crit):
        # 合并 real_crits 和 fake_crits 并转为 numpy 数组
