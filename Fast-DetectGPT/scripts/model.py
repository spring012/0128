...
def from_pretrained(cls, model_name, kwargs, cache_dir, max_retries=5, retry_interval=5):
        # use your local model if it exists
        ...
        while retries < max_retries:
            try:
                ...
                print(f"第 {retries + 1} 次尝试加载模型失败: {e}")
                retries += 1
                time.sleep(retry_interval)  # 等待重试

        raise Exception("达到最大重试次数，无法加载模型。")
